...
- **투자 성향 분석 (개선)**: 최신화된 7개 문항의 투자 성향 분석 퀴즈를 제공합니다. 긴 질문 목록으로 인한 불편을 해소하기 위해 **"이전/다음" 버튼을 통한 페이징 UI를 적용**하여 사용자 경험을 개선했습니다.
...
- **매매 복기 노트 (Trading Journal) (기능 강화)**: 모든 매매의 상세 내역을 기록하고 표로 관리할 수 있습니다.
  - **체계적인 매수 근거 관리**: **매수 근거 입력이 필수 항목으로 변경**되었습니다. 사용자는 **#PER, #PBR 등 주요 지표를 태그 형식으로 선택**하고, 상세한 내용을 직접 서술하여 자신의 투자 결정을 체계적으로 기록할 수 있습니다.
  - **향상된 가독성**: 기록된 매수 근거는 테이블에 `#EPS 외 2개`와 같은 요약된 뱃지 형태로 표시되며, **마우스를 올리면 툴팁으로 전체 태그와 상세 내용을 확인**할 수 있어 가독성이 향상되었습니다.
  - **편리한 날짜 입력**: 캘린더 팝업을 통해 매수일과 매도일을 직관적으로 선택할 수 있으며, 매도일이 매수일보다 빠르지 않도록 유효성을 검증합니다.
  - **다중 통화 및 기본값 설정**: **최초 방문 시 기본 통화가 KRW로 설정**되어 한국 사용자에게 더 직관적인 경험을 제공하며, 언제든지 KRW/USD로 전환할 수 있습니다.
  - **시각적 명확성**: 매도 정보가 없는 항목은 'N/A' 대신 아이콘으로 표시하여 가독성을 높였습니다.
- **마이페이지**: GitHub 계정 정보와 분석된 투자 성향을 확인하고, 안전하게 로그아웃할 수 있습니다.
- **매매 기록 심층 분석 (AI 기능 강화)**: 기록된 매매 내역을 바탕으로 성과를 분석하고 투자 결정을 복기할 수 있는 심층 분석 기능을 제공합니다.
  - **사용자 맞춤형 AI 분석 (Gemini)**: AI 분석 리포트에 **사용자가 직접 입력한 매수 근거(태그, 상세 내용)가 함께 반영**됩니다. 이를 통해 Gemini API가 **"✍️ 내 근거 분석"** 섹션을 추가하여, 사용자의 투자 논리가 타당했는지에 대한 맞춤형 피드백을 제공합니다.
